...
struct RGBA {
  ...
  Byte luminance() const { return 0.299 * r + 0.587 * g + 0.114 * b; }
...
public:
  ...
  RGBA &operator()(int x, int y) { return data[x + y * width]; }

  RGBA operator()(int x, int y) const { return data[x + y * width]; }
...
public:
  ...
  Byte &operator()(int x, int y) { return data[x + y * width]; }

  Byte operator()(int x, int y) const { return data[x + y * width]; }
...
inline void SaveHist(const GrayscaleImage &im, std::string filename,
                     float scale = 0.05) {
...
inline void SaveHist(const ColorImage &im, std::string filename,
                     float scale = 0.05) {
...
inline ColorImage::ColorImage(const GrayscaleImage &im) {
  width = im.GetWidth();
  height = im.GetHeight();
  data.resize(width * height);
...
inline int car(float val, int limit) {
